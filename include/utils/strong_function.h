...
namespace small_vector::utils
{
  ...
  struct strong_function<Tag,ret_type(args_type...)>
    ...
    template<typename Cb,
        std::enable_if_t<std::is_invocable_r_v<ret_type,Cb,args_type...>,bool> = true>
    explicit strong_function( Cb callable ) : callable_{callable}{}

    template<typename ... fargs_type>
    auto operator() ( fargs_type &&... args ) const
       noexcept(noexcept(callable_( std::forward<fargs_type>(args)...)))
