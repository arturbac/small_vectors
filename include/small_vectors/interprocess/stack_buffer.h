...
struct constexpr_index
  ...
  constexpr bool
    compare_exchange_strong(stack_index_t & e, stack_index_t i, [[maybe_unused]] std::memory_order m = seq_cst) noexcept
    {
    if(e == value) [[likely]]
      {
      value = i;
...
namespace detail
  ...
  constexpr auto footer_from_index(stack_buffer_t<buffer_size, index_type> & buff, stack_index_t six) noexcept
    {
    auto it_top_footer{
      ranges::next(ranges::begin(buff.buffer_), static_cast<ptrdiff_t>(six.index - sizeof(footer_data_t)))
    };
...
struct push_t
  ...
  small_vector_static_call_operator constexpr auto operator()(
    ...
    if(buff.stack_index_.compare_exchange_strong(stack_index, next_index, release)) [[likely]]
      ...
      auto ix_iter{
        ranges::copy(
          data_beg, data_end, ranges::next(ranges::begin(buff.buffer_), static_cast<ptrdiff_t>(stack_index.index))
        )
          .out
      };
