...
namespace algo::bwt
{
  namespace concepts
    ...
    concept char_type = std::same_as<std::remove_const_t<value_type>,char>
                      || std::same_as<std::remove_const_t<value_type>,char8_t>
  ...
  struct encode_t
    ...
    template<std::contiguous_iterator source_iterator, std::sentinel_for<source_iterator> sentinel,
    ...
    constexpr auto operator()(source_iterator beg, sentinel end, out_iterator out )
          small_vector_static_call_operator_const noexcept
          -> out_iterator
      {
      namespace ranges = std::ranges;
      using char_type = std::iter_value_t<source_iterator>;
      using string_type = coll::basic_string<char_type>;
      ...
      if( beg != end)
        ...
        coll::small_vector<uint32_t,uint32_t> rotvec;
        rotvec.resize(sz);
        std::iota( ranges::begin(rotvec), ranges::end(rotvec), 0u );
        ranges::sort( rotvec, [&f_view]( uint32_t lix, uint32_t rix) noexcept
                              { return f_view(lix).compare(f_view(rix)) < 0; });

        out = ranges::transform( rotvec, out,
                           [&table,sz]( uint32_t ix ) noexcept
                             -> char_type
                           {
    ...
    constexpr auto operator()( contiguous_range const & range, out_iterator out )
        small_vector_static_call_operator_const noexcept
      {
    ...
    constexpr auto operator()(source_iterator beg, sentinel end, out_iterator out )
      ...
      if( beg != end)
        ...
        if( it_x != ranges::end(btw_arr))
          ...
          out = ranges::transform( first, last, out,
                             [&btw_arr]( uint32_t ix) noexcept
                             {
                             return btw_arr[ix];
                             }).out;
          }
