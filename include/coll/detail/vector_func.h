...
namespace coll::detail
{
  ...
  inline constexpr std::string_view enum_name( vector_outcome_e value ) noexcept
    {
    switch(value)
      {
  ...
  concept value_type_or_lv_refrence =
    std::same_as<Expected, std::remove_cvref_t<T>>;
  //-------------------------------------------------------------------------------------------------------------------
  template<typename T,typename Expected>
  concept const_or_nonconst_pointer =
       std::same_as<T, Expected *>
    or std::same_as<T, Expected const *>;
  //-------------------------------------------------------------------------------------------------------------------
  template<typename vector_type, typename size_type>
  concept has_index_operator = 
    requires(vector_type const & vec, size_type index )
      {
  ...
  //-------------------------------------------------------------------------------------------------------------------
  template<typename vector_type>
  concept has_data_member_function = 
  ...
  struct internal_data_context_t 
    ...
    inline explicit constexpr internal_data_context_t( vector_type & vec ) noexcept
      : ctx{ vec.storage_.context() }, size_{ vec.size() }
      {}
    };
  ...
  erase( vector_type & vec,
       ...
       ) noexcept(std::is_nothrow_move_assignable_v<typename vector_type::value_type>)
        -> typename vector_type::iterator
    {
    using value_type = typename vector_type::value_type;
    using size_type = typename vector_type::size_type;
  ...
  inline constexpr void
  emplace_back_unchecked(vector_type & vec, Args &&... args)
      noexcept(std::is_nothrow_constructible_v<typename vector_type::value_type,Args...>)
    {
    internal_data_context_t const my{ vec };
  ...
  emplace_back(vector_type & vec, Args &&... args)
          ...
          if( new_space )
            ...
            if constexpr (use_nothrow)
              ...
              typename noexcept_if<std::is_nothrow_move_constructible_v<value_type>>::cond_destroy_at 
                el{ std::construct_at(unext(new_space.data(), my.size()), std::forward<Args>(args)...) };
  ...
  ///\warning if there is no space left for inserting all elements it returns \ref static_vector_outcome_e::out_of_storage
  ...
  inline constexpr vector_outcome_e
  insert(vector_type & vec, typename vector_type::const_iterator citpos,
         source_iterator itbeg, source_iterator itend ) 
      noexcept(is_nothrow_move_constr_and_constr_v<typename vector_type::value_type,
        ...
        if(new_capacity != 0u)
          ...
          if( new_space )
            ...
            typename noexcept_if<std::is_nothrow_move_constructible_v<value_type>>::cond_destroy_range
              ext_range_unwind{ new_space.data(), lower_el_count, nic_sum(lower_el_count,u_new_el_count) };
  ...
  inline constexpr void
  emplace_unchecked_impl(vector_type & vec, internal_data_context_t<vector_type> const & my,
                    typename vector_type::const_iterator citpos, Args &&... args)
      noexcept(is_nothrow_move_constr_and_constr_v<typename vector_type::value_type, Args...>)
    {
    using value_type = typename vector_type::value_type;
    using size_type = typename vector_type::size_type;
  ...
  template<typename vector_type, typename ...Args>
    requires emplace_constraints<vector_type,Args...>
  inline constexpr vector_outcome_e
  emplace(vector_type & vec, typename vector_type::const_iterator citpos, Args &&... args)
      noexcept(is_nothrow_move_constr_and_constr_v<typename vector_type::value_type,Args...>)
    {
    constexpr bool use_nothrow = is_nothrow_move_constr_and_constr_v<typename vector_type::value_type,Args...>;
  ...
  relocate_elements_dyn( vector_type & vec, internal_data_context_t<vector_type> const & my,
                         typename vector_type::size_type new_capacity)
  ...
  template<typename vector_type>
    requires reserve_constraints<vector_type>
  constexpr vector_outcome_e
  relocate_elements_static( vector_type & vec, internal_data_context_t<vector_type> const & my)
      noexcept(std::is_nothrow_move_constructible_v<typename vector_type::value_type>)
    {
    constexpr bool use_nothrow = std::is_nothrow_move_constructible_v<typename vector_type::value_type>;
  ...
  template<typename vector_type>
    requires reserve_constraints<vector_type>
  constexpr vector_outcome_e
  reserve( vector_type & vec, typename vector_type::size_type new_capacity )
    noexcept(std::is_nothrow_move_constructible_v<typename vector_type::value_type>)
    {
