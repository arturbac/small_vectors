...
"test_semaphore"_test = [&]
  ...
  std::unique_ptr<bip::interprocess_semaphore,destruction_only> 
    reader_finished_semaphore{ip::construct_at<semaphore_decl>(region, 0)},
  ...
  auto writter_done{ip::construct_at<writter_done_decl>(region, false)};
    ...
    auto writter = ip::fork([](std::string_view shared_mem_name )
      {
  ...
  auto reader = ip::fork([](std::string_view shared_mem_name )
    {
     bip::shared_memory_object shm_obj{ bip::open_only, shared_mem_name.data() , bip::read_write };
     bip::mapped_region cregion{ shm_obj, bip::read_write };
     decltype(auto) stack { ip::ref<stack_decl>(cregion)};
     ...
     while(!writter_done_obj || !ip::empty(stack))
      ...
      if(ip::empty(stack))
        ...
        if(succeed_range_valid == popres)
          ...
          ranges::subrange refsubrange{ranges::begin(refvalue), ranges::next(ranges::begin(refvalue), static_cast<ptrdiff_t>(indata.size()))};
