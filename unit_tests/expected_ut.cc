...
using value_type_list
  = metatests::type_list<int32_t, uint32_t, void, non_trivial, non_trivial_ptr, non_trivial_ptr_except>;
using value_type_non_void_list
  = metatests::type_list<int32_t, uint32_t, non_trivial, non_trivial_ptr, non_trivial_ptr_except>;
...
constexpr test_error operator++(test_error v) noexcept { return static_cast<test_error>(static_cast<int>(v) + 1); }
...
struct implicit_test_t
  {
  template<typename I>
  constexpr auto operator()(I && i) const noexcept -> expected_return_type
    {
...
consteval bool test_ref()
  ...
  return std::is_same_v<decltype(v.value()), int &> && std::is_same_v<decltype(*v), int &>
         && std::is_same_v<decltype(v2.value()), void> && std::is_same_v<decltype(*v2), void>;
  }
...
consteval bool test_const_ref()
  {
  expected<int, int> const v{};
  return std::is_same_v<decltype(v.value()), int const &> && std::is_same_v<decltype(*v), int const &>;
  }
...
consteval bool test_ref()
  ...
  expected<int, int> v{};
  expected<void, int> v2{};
  return std::is_same_v<decltype(v.error()), int &> && std::is_same_v<decltype(v2.error()), int &>;
  }
...
consteval bool test_const_ref()
  {
  expected<int, int> const v{};
  expected<void, int> const v2{};
  return std::is_same_v<decltype(v.error()), int const &> && std::is_same_v<decltype(v2.error()), int const &>;
  }
  ...
  "unexpected_constr"_test = [&]
  {
    auto fn_tmpl = []<typename error_type>(error_type const *) -> metatests::test_result
    {
      using unexpected_type = unexpected<error_type>;
        ...
        {
        constexpr_test(
          std::add_const_t<unexpected_type>{static_cast<error_type>(2)}.error() == static_cast<error_type>(2)
        );
        }
  ...
  "expected and_then"_test = [&]
  {
      ...
      = []<typename value_type, typename error_type>(value_type const *, error_type const *) -> metatests::test_result
    {
      ...
      auto f = [](value_type v) noexcept { return expected_type{in_place, ++v}; };
        {
        expected_type const ex{in_place, value_type{2}};
        auto res{ex.and_then([](value_type v) { return expected_type{in_place, ++v}; })};
