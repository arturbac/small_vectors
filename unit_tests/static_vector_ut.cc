...
struct non_trivial_counter : public non_trivial
  ...
  non_trivial_counter(int value) noexcept : non_trivial{value}, has_resource{true} { ++counter_; }

  non_trivial_counter(non_trivial_counter && r) noexcept : non_trivial{std::move(r)}, has_resource{r.has_resource}
    {
    if(r.has_resource)
      r.has_resource = false;
    }

  non_trivial_counter(non_trivial_counter const & r) noexcept : non_trivial{r}, has_resource{r.has_resource}
    {
    if(has_resource)
      ++counter_;
    }
...
using traits_list = metatests::type_list<
  uint16_t,
  int32_t,
  int64_t,
  double,
...
namespace concepts
  ...
  static_assert(same_as_static_vector<static_vector<uint8_t, 7> const>);
...
test_result consteval consteval_static_vector_deduced_types()
  ...
  vec_type const & vc{v};
  vec_type & vr{v};
  test_result res = constexpr_test(std::same_as<value_type &, decltype(at(v, 0u))>)
...
int main()
  ...
  "test_static_vector_basic"_test = [&result]
  {
    auto fn_tmpl = []<typename value_type>(value_type const *) -> metatests::test_result
    {
      ...
      vector_type vec;
      tr |= constexpr_test(size(vec) == 0) | constexpr_test(capacity(vec) == elements)
            | constexpr_test(free_space(vec) == elements) | constexpr_test(begin(vec) == end(vec));

      tr |= constexpr_test(std::same_as<vector_outcome_e, decltype(emplace_back(vec, 1))>);

      auto res = emplace_back(vec, 1);
      tr |= constexpr_test(res == no_error) | constexpr_test(size(vec) == 1)
            | constexpr_test(free_space(vec) == elements - 1) | constexpr_test(vec[0u] == 1);

      emplace_back<unchecked>(vec, 2);
      tr |= constexpr_test(std::same_as<void, decltype(emplace_back<unchecked>(vec, 1))>);

      tr |= constexpr_test(size(vec) == 2) | constexpr_test(free_space(vec) == elements - 2)
            | constexpr_test(vec[0u] == 1) | constexpr_test(vec[1u] == 2);

      res = emplace_back(vec, 3);
      tr |= constexpr_test(res == no_error) | constexpr_test(size(vec) == 3)
            | constexpr_test(free_space(vec) == elements - 3) | constexpr_test(vec[0u] == 1)
            | constexpr_test(vec[1u] == 2) | constexpr_test(vec[2u] == 3);

      res = vec.emplace_back(4);
      tr |= constexpr_test(res == no_error) | constexpr_test(size(vec) == 4)
            | constexpr_test(free_space(vec) == elements - 4)
            | constexpr_test(std::distance(begin(vec), end(vec)) == size(vec)) | constexpr_test(vec[0u] == 1)
            | constexpr_test(vec[1u] == 2) | constexpr_test(vec[2u] == 3) | constexpr_test(vec[3u] == 4);
    ...
    auto fn_tmpl = []<typename value_type>(value_type const *) -> metatests::test_result
    {
      ...
      vector_type vec;
        ...
        tr |= constexpr_test(std::same_as<vector_outcome_e, decltype(emplace(vec, begin(vec), v))>)
              | constexpr_test(res == no_error) | constexpr_test(std::distance(begin(vec), end(vec)) == size(vec))
              | constexpr_test(size(vec) == 1) | constexpr_test(equal(vec, expected));
        }
        {
        ...
        tr |= constexpr_test(size(vec) == 2) | constexpr_test(equal(vec, expected));
        }
        {
        std::array<value_type, 3> expected{2, 3, 1};
        value_type v = 3;
        auto res = vec.emplace(begin(vec) + 1, std::move(v));
        tr |= constexpr_test(res == no_error) | constexpr_test(size(vec) == 3) | constexpr_test(equal(vec, expected));
        }
        {
        std::array<value_type, 4> expected{2, 4, 3, 1};
        value_type v = 4;
        auto res = emplace(vec, begin(vec) + 1, std::move(v));
        tr |= constexpr_test(res == no_error) | constexpr_test(size(vec) == 4) | constexpr_test(equal(vec, expected));
        }
        {
        std::array<value_type, 5> expected{2, 4, 3, 1, 5};
        value_type v = 5;
        auto res = emplace(vec, end(vec), std::move(v));
        tr |= constexpr_test(res == no_error) | constexpr_test(size(vec) == 5) | constexpr_test(equal(vec, expected));
        }
      return tr;
    };
  ...
  "test_static_vector_insert"_test = [&result]
  {
    auto fn_tmpl = []<typename value_type>(value_type const *) -> metatests::test_result
    {
      ...
      vector_type vec;
        ...
        tr |= constexpr_test(res == vector_outcome_e::no_error) | constexpr_test(size(vec) == 5)
              | constexpr_test(equal(vec, tst));
        }
        {
        ...
        auto res = insert(vec, begin(vec), begin(tst), end(tst));
        tr |= constexpr_test(res == vector_outcome_e::no_error) | constexpr_test(size(vec) == 8)
        ...
        tr |= constexpr_test(res == vector_outcome_e::no_error) | constexpr_test(size(vec) == 10)
              | constexpr_test(equal(vec, exp));
        }
        {
        ...
        tr |= constexpr_test(res == vector_outcome_e::no_error) | constexpr_test(size(vec) == 14)
              | constexpr_test(equal(vec, exp));
        }
        {
        ...
        tr |= constexpr_test(res == vector_outcome_e::no_error) | constexpr_test(size(vec) == 18)
              | constexpr_test(equal(vec, exp));
        }
        {
        ...
        //     dump(vec);
        tr |= constexpr_test(res == vector_outcome_e::no_error) | constexpr_test(size(vec) == 20)
              | constexpr_test(equal(vec, exp));
        }
        {
        std::array<value_type, 2> tst{51, 52};
        ...
        auto res = insert(vec, end(vec), begin(tst), end(tst));
        //     dump(vec);
        tr |= constexpr_test(res == vector_outcome_e::out_of_storage) | constexpr_test(size(vec) == 20)
              | constexpr_test(equal(vec, exp));
        }
      return tr;
    };
  ...
  "test_static_vector_erase_at_end"_test = [&result]
  {
    auto fn_tmpl = []<typename value_type>(value_type const *) -> metatests::test_result
    {
      ...
      tr |= constexpr_test(size(vec) == 0) | constexpr_test(end(vec) == res);

      insert(vec, end(vec), begin(tst), end(tst));
      res = vec.erase_at_end(begin(vec) + 5);
      tr |= constexpr_test(size(vec) == 5) | constexpr_test(begin(vec) + 5 == res)
  ...
  "test_static_vector_erase_range"_test = [&result]
  {
    auto fn_tmpl = []<typename value_type>(value_type const *) -> metatests::test_result
    {
      ...
      tr |= constexpr_test(size(vec) == 18) | constexpr_test(res == begin(vec) + 2) | constexpr_test(at(vec, 2u) == 13)
            | constexpr_test(at(vec, 3u) == 1) | constexpr_test(at(vec, 4u) == 31) | constexpr_test(at(vec, 17u) == 52);

      res = vec.erase(begin(vec) + 3, begin(vec) + 3);
      tr |= constexpr_test(size(vec) == 18) | constexpr_test(res == begin(vec) + 3) | constexpr_test(at(vec, 2u) == 13)
            | constexpr_test(at(vec, 3u) == 1) | constexpr_test(at(vec, 4u) == 31) | constexpr_test(at(vec, 17u) == 52);
  ...
  "test_static_vector_erase"_test = [&result]
  {
    auto fn_tmpl = []<typename value_type>(value_type const *) -> metatests::test_result
    {
      ...
      tr |= constexpr_test(size(vec) == 19) | constexpr_test(res == begin(vec) + 2) | constexpr_test(at(vec, 2u) == 22)
            | constexpr_test(at(vec, 3u) == 13) | constexpr_test(at(vec, 4u) == 1) | constexpr_test(at(vec, 18u) == 52);

      res = erase(vec, end(vec) - 1);
      tr |= constexpr_test(res == end(vec)) | constexpr_test(size(vec) == 18) | constexpr_test(at(vec, 17u) == 51);

      res = vec.erase(begin(vec));
      tr |= constexpr_test(res == begin(vec)) | constexpr_test(size(vec) == 17) | constexpr_test(at(vec, 0u) == 12)
  ...
  "test_static_vector_split_by_half"_test = [&result]
  {
    auto fn_tmpl = []<typename value_type>(value_type const *) -> metatests::test_result
    {
      test_result tr;
        ...
        tr |= constexpr_test(res == vector_outcome_e::no_error) | constexpr_test(size(vec) == 10)
              | constexpr_test(size(vec2) == 10) | constexpr_test(equal(vec, std::span(begin(tst), 10)))
        ...
        auto res = split_by_half(vec, vec2);
        tr |= constexpr_test(res == vector_outcome_e::no_error) | constexpr_test(size(vec) == 10)
              | constexpr_test(size(vec2) == 20) | constexpr_test(equal(vec, std::span(begin(tst), 10)))
